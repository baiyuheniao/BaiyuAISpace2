...
- **未来计划**：在条件允许时，新建一个独立的子项目，专门面向 Android 平台的适配与发布。
- **功能裁剪**：当前主项目将移除或不再优先实现对移动端非常消耗性能的本地部署功能（例如大规模本地模型推理、完整离线向量检索、重度离线 Embedding 处理等）。
- **原因**：移动设备资源受限，移动端应依赖轻量化或服务化方案；把高性能本地能力放到专门子项目或云端实现，可提高维护性与用户体验。
- **迁移与兼容**：保留必要的接口与配置（API、数据导入/导出、知识库抽象等），以便未来将部分功能迁移到 Android 子项目或由云端替代实现。
- **建议实现路径**：优先考虑服务化后端或使用 Capacitor / React Native 等混合方案配合远程后端，避免在手机端做大规模本地推理；若确需本地化，建议另起子项目并选用轻量模型与移动专用推理引擎。

### 📌 后续事项

- 在 README / 新项目模板中注明 Android 子项目的责任范围与限制。
- 在主项目中标注已裁剪的功能点，便于将来回收或迁移。
